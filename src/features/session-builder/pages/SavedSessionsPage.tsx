  ...
  return <div className="page saved-sessions-page">
    ...
    {drafts.length ? <section className="saved-session-grid" aria-label="Borradores guardados">{drafts.map((draft) => {
      ...
      return <article className="saved-session-card" key={draft.id}>
        ...
        <footer><Link className="text-link" to={`/sessions/prepare/${draft.id}`}>Abrir borrador</Link><button type="button" onClick={() => duplicate(draft.id)}>Duplicar</button><button className="saved-session-card__delete" type="button" onClick={() => setPendingDeletion(draft.id)}>Eliminar</button></footer>
